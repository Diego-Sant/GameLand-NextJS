...
const Products = () => {
    ...
    const handleChangeGameMode = (e) => {
        ...
        if (isChecked) {
            setSelectedGameModes([...selectedGenres, value]);
        } else {
            setSelectedGameModes(selectedGameModes.filter((item) => item !== value));
        }
    };
...
const poster = [
    {
        id: 1,
        img: "https://images.igdb.com/igdb/image/upload/t_original/ar2d8r.jpg",
  ...
  return (
    <MainLayout>
        ...
        <h2 className='font-bold mb-2'>Categorias</h2>
            ...
            {gameModes?.map((item) => (
                <div key={item.id}>
                    <input type="checkbox" className='outline-none mr-1' id={item.id} value={item.id} onChange={handleChangeGameMode} />
            ...
            <div className='flex-[3]'>
                {poster.map((item) => (
                    <Link href="/produto/2" key={item.id} className='md:flex justify-center items-center hidden cursor-pointer hover:brightness-95'><img className='bg-cover mb-[50px] xl:max-w-[880px] xl:min-w-[880px] 2xl:max-w-[1100px] 2xl:min-w-[1100px] 3xl:max-w-[1300px] 3xl:min-w-[1300px]' src={item.img} alt={item.title} /></Link>
                ))}
                <h1 className='flex items-center justify-center mb-5 text-3xl font-bold'>Playstation</h1>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 justify-items-center">
                    {products && shuffleArray(products)
                    .filter((item) => item.consoles.includes("Playstation"))
                    ...
                    .filter((item) => selectedGameModes.length === 0 || selectedGameModes.some((gameModeId) => item.gameMode.includes(gameModes.find((gameMode) => gameMode.id === parseInt(gameModeId)).gameMode)))
